                                ...
                                RokoReferrals.activateDiscountWithCode(referralCode, new RokoReferrals.OnActivateDiscountWithCode() {
                                    ...
                                    public void success(final ResponseActivatedDiscount responseActivatedDiscount) {
                                        runOnUiThread(new Runnable() {
                                            @Override
                                            public void run() {
                                                getSharedPreferences("discountID", Context.MODE_PRIVATE).edit().putLong("discount", responseActivatedDiscount.data.discount.objectId).apply();
                                                Toast.makeText(getApplicationContext(), responseActivatedDiscount.data.discount.objectId + " Activated", Toast.LENGTH_SHORT).show();
                                            }
                                        });
                                    }
